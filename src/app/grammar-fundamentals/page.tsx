...
export default function GrammarFundamentalsPage() {
  const { user } = useAuth();
  const [chapters, setChapters] = useState<Chapter[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const path = 'vocabulary'; // This path still points to the correct Firestore document ID
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Grammar Fundamentals</h1>
      ...
      {loading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
         ...
         <p className="text-center text-muted-foreground">No chapters available for this section yet.</p>
      )}
    </div>
  );
}
